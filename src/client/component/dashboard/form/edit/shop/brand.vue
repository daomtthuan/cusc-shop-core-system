...
<script lang="ts">
  ...
  @Component({
    name: 'component-dashboard-form-edit-shop-brand',
  ...
  export default class extends mixins(validationMixin) {
    ...
    private form: App.Form.Edit.Shop.Brand = {
      name: null,
      state: null,
    };
    ...
    public async fetch() {
      try {
        let brands: Entity.Brand[] = (await this.$axios.get('/api/admin/brand', { params: { id: this.id } })).data;
        if (brands.length == 1) {
          this.form.name = brands[0].name;
          this.form.state = brands[0].state;
        } else {
    ...
    public async submit() {
      ...
      try {
        this.pending = true;
        await this.$axios.put('/api/admin/brand', this.form, { params: { id: this.id } });

        this.$nuxt.$bvToast.toast('Thông tin thương hiệu đã được chỉnh sửa.', {
