        ...
        <div v-else-if="!$fetchState.error">
          ...
          <b-form @click.prevent="submit" class="text-center">
            <b-button type="submit" variant="primary" :disabled="pending">
              <span v-if="!pending">Xác nhận đã thanh toán</span>
...
<script lang="ts">
  ...
  @Component({
    name: 'page-dashboard-shop-bill-confirm',
    head: {
      title: 'Bảng điều khiển - Cửa hàng - Đơn hàng - Xác nhận thanh toán',
    },
    methods: { toMoney },
  })
  export default class extends Vue {
    ...
    public async submit() {
      try {
        this.pending = true;
        await this.$axios.put('/api/admin/bill', { status: 2 }, { params: { id: this.idBill } });

        this.$nuxt.$bvToast.toast('Đã xác nhận thanh toán đơn hàng.', {
